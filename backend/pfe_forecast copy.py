...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    stats = estimate_gbm_parameters(market_df, sentiment_column)
    ...
    print("\n=== Market data ===")
    print(
        f"Sample size: {len(market_df):d} observations from {market_df.index[0].date()} to {market_df.index[-1].date()}"
    )
